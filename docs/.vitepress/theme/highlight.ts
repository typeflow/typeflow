import { tokenize, type Token } from "@thomasfarineau/typeflow-parser";

const KEYWORDS = new Set(["input", "map", "from"]);
...
function classify(tokens: Token[], idx: number): string {
  ...
  switch (t.type) {
    ...
    case "ident": {
      ...
      if (
        (prev === undefined || (prev.type === "punct" && (prev.value === "{" || prev.value === ","))) &&
        next?.type === "punct" &&
        (next.value === ":" || (next.value === "?" && tokens[idx + 2]?.value === ":"))
      ) {
        return "tok-prop";
      }
...
export function highlightJson(source: string): string {
  ...
  while (i < n) {
    ...
    if (c === '"') {
      ...
      html += span(source[k] === ":" ? "tok-prop" : "tok-string", source.slice(i, j));
      i = j;
      continue;
    }
    const rest = source.slice(i);
    const num = c === "-" || (c >= "0" && c <= "9") ? rest.match(JSON_NUM) : null;
    if (num) {
