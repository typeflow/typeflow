...
const jqText = ref(DEFAULT_JQ);

const dirty = computed(() => jqText.value !== DEFAULT_JQ);

function reset() {
  jqText.value = DEFAULT_JQ;
}
...
const highlightJq = (source: string) => escapeHtml(source);

const converted = computed(() => convertJq(jqText.value));

const typeflowHtml = computed(() =>
  converted.value.ok
    ? highlightTypeflow(converted.value.typeflow.trimEnd())
    : escapeHtml('# Fix the jq filter to see the conversion.'),
);

const LINE_HEIGHT = 20.8;
const V_PADDING = 24;
const jqHeight = computed(() => {
  const lines = jqText.value.split('\n').length + 1;
      ...
      <section class="jp-pane">
        <header>
          ...
          <span
            ...
            {{ converted.errors.length ? 'x' : 'ok' }}
          </span>
          <button v-if="dirty" class="jp-reset" @click="reset">Reset</button>
        </header>
        <div class="jp-editor" :style="{ height: jqHeight }">
          <CodeEditor v-model="jqText" :highlight="highlightJq" />
        </div>
      </section>
      <section class="jp-pane">
