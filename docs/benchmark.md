...
Same transformation, four implementations, measured **in your browser, on your machine** — no pre-baked numbers. Each scenario is written as a Typeflow mapping, an equivalent jq filter converted to Typeflow, an equivalent JSONata expression, and a hand-written JavaScript function (the native ceiling). Pick an array size, hit run, and compare.
...
- **One-time costs are separated.** Typeflow is compiled once, jq is converted+compiled once, and JSONata parsed once — what the bars compare is per-call execution, which is what matters when a mapping runs on every request. The one-time cost is reported under the results.
...
- JSONata v2's `evaluate()` is asynchronous, so its numbers include the promise overhead inherent to its API. Typeflow, converted jq, and the native function are synchronous.
