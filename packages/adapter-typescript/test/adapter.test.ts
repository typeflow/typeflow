...
import { typeToString, type Type } from "@thomasfarineau/typeflow-core";
import { createTypeScriptResolver } from "../src/index.ts";
...
function resolveOk(typeName: string): Type {
  const result = resolver({ typeName, from: "./fixtures/shapes", filePath: here });
...
describe("typescript adapter", () => {
  ...
  test("reports unknown type names with available alternatives", () => {
    const result = resolver({ typeName: "Nope", from: "./fixtures/shapes", filePath: here });
    expect("error" in result && result.error).toContain("Person");
  });

  test("reports unresolvable modules", () => {
    const result = resolver({ typeName: "X", from: "./does-not-exist", filePath: here });
