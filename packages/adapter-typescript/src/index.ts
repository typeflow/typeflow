...
import { makeUnion, T, type Type } from "@thomasfarineau/typeflow-core";
import type { ResolveTypeRequest, TypeResolver } from "@thomasfarineau/typeflow-compiler";
...
export function createTypeScriptResolver(): TypeResolver {
  const cache = new Map<string, { program: ts.Program; checker: ts.TypeChecker }>();
  ...
  return (req: ResolveTypeRequest) => {
    ...
    if (!moduleFile) {
      return { error: `module '${req.from}' was not found (resolved from '${baseDir}').` };
    }
    ...
    if (!decl) {
      ...
      const hint = available.length ? ` Available types: ${available.join(", ")}.` : "";
      return { error: `type '${req.typeName}' is not declared in '${req.from}'.${hint}` };
    }
...
export function resolveTypeScriptType(req: ResolveTypeRequest): { type: Type } | { error: string } {
  return createTypeScriptResolver()(req);
}

type NamedTypeDecl = ts.InterfaceDeclaration | ts.TypeAliasDeclaration;

function findTypeDeclaration(sourceFile: ts.SourceFile, name: string): NamedTypeDecl | undefined {
...
function listTypeNames(sourceFile: ts.SourceFile): string[] {
  const names: string[] = [];
  for (const stmt of sourceFile.statements) {
    if (ts.isInterfaceDeclaration(stmt) || ts.isTypeAliasDeclaration(stmt)) names.push(stmt.name.text);
  }
  return names;
}
...
function convertType(
  ...
  if (flags & ts.TypeFlags.StringLiteral) return T.literal((tsType as ts.StringLiteralType).value);
  if (flags & ts.TypeFlags.NumberLiteral) return T.literal((tsType as ts.NumberLiteralType).value);
  if (flags & ts.TypeFlags.BooleanLiteral) {
    return T.literal((tsType as unknown as { intrinsicName: string }).intrinsicName === "true");
  }
  ...
  if (tsType.isUnion()) {
    return makeUnion(tsType.types.map((t) => convertType(t, checker, location, visiting, depth + 1)));
  }

  if (checker.isArrayType(tsType)) {
    const args = checker.getTypeArguments(tsType as ts.TypeReference);
    const element = args[0] ? convertType(args[0], checker, location, visiting, depth + 1) : T.unknown;
    return T.array(element);
  }

  if (checker.isTupleType(tsType)) {
    const args = checker.getTypeArguments(tsType as ts.TypeReference);
    const element = makeUnion(args.map((t) => convertType(t, checker, location, visiting, depth + 1)));
  ...
  if (flags & (ts.TypeFlags.Object | ts.TypeFlags.Intersection)) {
    ...
    try {
      const fields = props.map((prop) => {
        ...
        let converted = convertType(propType, checker, location, visiting, depth + 1);
...
function stripUndefinedMember(t: Type): Type {
  ...
  if (t.kind === "union") return makeUnion(t.types.filter((x) => x.kind !== "undefined"));
  return t;
}
