...
export function typeKey(t: Type): string {
  switch (t.kind) {
    ...
    case "object":
      return `obj:{${t.fields
        .map((f) => `${JSON.stringify(f.name)}${f.optional ? "?" : ""}:${typeKey(f.type)}`)
        .join(",")}}`;
    case "union":
      return `uni:${t.types.map(typeKey).sort().join("|")}`;
    default:
      return t.kind;
  }
}
...
export function makeUnion(types: Type[]): Type {
  ...
  for (const t of flat) {
    ...
    if (t.kind === "literal") {
      ...
      if (typeof t.value === "boolean" && (hasBoolean || (hasTrue && hasFalse))) continue;
    }
    const key = typeKey(t);
...
export function stripNullish(t: Type): Type {
  ...
  if (t.kind === "union") {
    const rest = t.types.filter((x) => x.kind !== "null" && x.kind !== "undefined");
...
export function typeToString(t: Type, indent = ""): string {
  ...
  const fields = t.fields
    .map((f) => `${inner}${fieldName(f.name)}${f.optional ? "?" : ""}: ${typeToString(f.type, inner)};`)
    .join("\n");
  return `{\n${fields}\n${indent}}`;
}
...
function typeToInline(t: Type): string {
  switch (t.kind) {
    ...
    case "object":
      ...
      return `{ ${t.fields
        .map((f) => `${fieldName(f.name)}${f.optional ? "?" : ""}: ${typeToInline(f.type)}`)
        .join("; ")} }`;
    case "union":
