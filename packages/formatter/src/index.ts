...
import { parse, tokenize, type Token } from "@thomasfarineau/typeflow-parser";
...
function precOf(e: Expr): number {
  switch (e.kind) {
    case "cond": return 1;
    case "binary": return BINARY_PREC[e.op]!;
    case "unary": return 9;
    case "member":
    case "bracket":
    case "index":
...
class Printer {
  ...
  constructor(
    ...
    this.comments = tokenize(source, { includeComments: true, tolerant: true }).filter(
      (t) => t.type === "comment",
    );
  }
  ...
  print(): string {
    ...
    if (this.ast.input) {
      prevEnd = this.emitComments(lines, this.ast.input.span.start, prevEnd, "");
  ...
  private emitComments(lines: string[], offset: number, prevEnd: number, pad: string): number {
    while (this.nextComment < this.comments.length && this.comments[this.nextComment]!.start < offset) {
  ...
  private printObject(obj: ObjectExpr, indent: number, forceMultiline = false): string {
    if (obj.props.length === 0) return "{}";

    if (!forceMultiline && !this.hasCommentWithin(obj.span.start, obj.span.end)) {
      const parts = obj.props.map((p) => `${propName(p.name)}: ${this.printExpr(p.value, 1, indent)}`);
      const inline = `{ ${parts.join(", ")} }`;
      if (!inline.includes("\n") && indent + inline.length <= INLINE_LIMIT) return inline;
    }
    ...
    for (const prop of obj.props) {
      ...
      prevEnd = this.emitComments(lines, prop.span.start, prevEnd, pad);
      if (lines.length === before && this.hasBlankBetween(prevEnd, prop.span.start)) lines.push("");
      lines.push(`${pad}${propName(prop.name)}: ${this.printExpr(prop.value, 1, indent + 2)},`);
      prevEnd = prop.span.end;
    }
  ...
  private printExprInner(e: Expr, indent: number): string {
    switch (e.kind) {
      ...
      case "project":
        return `${this.printExpr(e.object, 10, indent)} -> ${this.printObject(e.body, indent)}`;
      case "unary": {
    ...
    switch (node.kind) {
      ...
      case "object": {
        ...
        const parts = node.fields.map(
          (f) => `${propName(f.name)}${f.optional ? "?" : ""}: ${this.printType(f.type, indent)}`,
        );
        const inline = `{ ${parts.join(", ")} }`;
        if (
          !inline.includes("\n") &&
        ...
        for (const field of node.fields) {
          ...
          lines.push(`${pad}${propName(field.name)}${field.optional ? "?" : ""}: ${this.printType(field.type, indent + 2)},`);
