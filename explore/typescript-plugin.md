...
{
  "compilerOptions": {
    "plugins": [{ "name": "typeflowjs/ts-plugin" }],
  },
}
```
...
Subpath of the existing package `typeflowjs/ts-plugin` (not
...
(`typeflowjs/ts-plugin`) — not a new separate npm package,
...
`"plugins": [{ "name": "typeflowjs/ts-plugin" }]` to a
...
with the Bun plugin (`typeflowjs/plugin`, preloaded via
...
module), exposed as a subpath `typeflowjs/node-loader`.
...
`examples/bun-plugin/`, subpath `typeflowjs/plugin`) was
